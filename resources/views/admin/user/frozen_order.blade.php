    ...
    <div class="card shadow mb-4 section_1">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <div class="d-flex flex-column">
                <h6 class="m-0 font-weight-bold text-primary fs-5" id="tittle">Đóng băng đơn hàng của người dùng <i class="text-danger">{{ $user->full_name }}</i></h6>
    ...
    <section class="container-fluid">
        <form action="{{ route('user.frozen.order',['user'=>$user->id]) }}" method="post" id="form">
            ...
            <div class="mt-2 fw-bold">
                ...
                <input type="text" name="custom_price" id="custom_price" value="{{ old('custom_price',"") }}" class="form-control" placeholder="Nhập giá giả">
                @error('custom_price')
                <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
