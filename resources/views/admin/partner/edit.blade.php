...
@vite('resources/js/admin/partner/edit.js')
...
<div class="container-fluid">
    ...
    <partner class="container-fluid">
        <form action="{{ route('partner.update',['partner'=>$partner->id]) }}" method="post" enctype="multipart/form-data" id="form">
            @csrf
            @method('PUT')
            <div class="fw-bold">
                <label for="">Tên đối tác</label>
                <input type="text" name="name" value="{{ old('name',$partner->name) }}" class="form-control" placeholder="Nhập tên partner (VD: 30/4-1/5)">
                @error('name')
            ...
            <div class="mt-2 fw-bold">
                ...
                <input type="text" name="link" value="{{ old('link',$partner->link) }}" class="form-control" placeholder="Nhập đường dẫn cho đối tác VD: https://www.facebook.com/">
                @error('link')
                <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
