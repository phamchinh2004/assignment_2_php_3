    ...
    <div class="order-detail-container">
        ...
        <div class="order-detail-card">
            <div class="order-detail-header">
                <div>
                    <h2 style="margin: 0; font-size: 24px; font-weight: 600;color:#000;">Chi tiết đơn hàng</h2>
                    <!-- Current Balance -->
                ...
                @if($frozen_order->snapshot_payment_method)
                    <div class="info-row">
                        ...
                        <div class="info-value" style="font-weight: 600;">
                            ...
                            @else
                                <span
                                    style="background: #ffc107; color: #333; padding: 4px 8px; border-radius: 4px; font-size: 12px; margin-left: 8px;">Chưa
                                    thanh toán</span>
                            @endif
                        </div>
                    </div>
            ...
            @if($frozen_order->tracking_number)
                <div class="info-section">
                    ...
                    <div class="info-row">
                        ...
                        <div class="info-value" style="font-family: monospace; font-weight: 600;">
                            {{ $frozen_order->tracking_number }}</div>
                    </div>
            ...
            @if($frozen_order->snapshot_api)
                <div class="info-section">
                    ...
                    <div class="info-row">
                        <div class="info-label">Đường dẫn theo dõi:</div>
                        <div class="info-value" style="display: flex; align-items: center; gap: 8px;">
                            <span
                                style="font-family: monospace; word-break: break-all; background: #f8f9fa; padding: 8px; border-radius: 4px; flex: 1;">
                                {{ $apiUrl }}
                            </span>
                            <button type="button" class="btn-copy-api" data-api="{{ $apiUrl }}"
                                title="Sao chép đường dẫn theo dõi">
                        ...
                        @foreach($allStatusesWithHistory as $index => $statusData)
                            ...
                            @if($isSpecial && ($statusData['specialType'] ?? '') === 'commission_paid')
                                ...
                                <div class="timeline-item {{ $isPaid ? 'completed' : 'pending' }} {{ $isPending ? 'is-muted' : '' }}">
                                    <div class="timeline-dot" @if($isPaid) style="background: #198754; border-color: #198754;"
                                    @elseif($isWaiting) style="background: #ffc107; border-color: #ffc107;" @else
                                        style="background: #dee2e6; border-color: #dee2e6;" @endif></div>
                            ...
                            @else
                                ...
                                <div
                                    class="timeline-item {{ $isCompleted ? 'completed' : ($isCurrent ? 'active' : ($isReached ? '' : 'pending')) }} {{ !$isReached ? 'is-muted' : '' }}">
                                    <div class="timeline-dot" @if($isReached && $status && $status->color)
                                    style="background: {{ $status->color }}; border-color: {{ $status->color }};" @else
                                        style="background: #dee2e6; border-color: #dee2e6;" @endif></div>
                                    <div class="timeline-content">
                                        <div class="timeline-label {{ !$isReached ? 'is-muted' : '' }}">
                                            ...
                                            @if($isReached && $statusOrder)
                                                @if($statusOrder->changedBy)
                                                    <span style="color: #666; font-size: 12px; font-weight: normal;">
                                                        (bởi
                                                        {{ $statusOrder->changedBy->full_name ?? $statusOrder->changedBy->username ?? 'Hệ thống' }})
                                                    </span>
                                                @else
                                                    <span style="color: #666; font-size: 12px; font-weight: normal;">
            ...
            @if(($frozen_order->status ?? 'pending') === 'pending')
                <div class="action-buttons">
                    ...
                    @else
                        ...
                        <button type="button" class="btn-report" id="btn_report_fake_order" @if($frozen_order->orderReport) disabled
                        @endif
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const copyButtons = document.querySelectorAll('.btn-copy-api');
                copyButtons.forEach(function (btnCopyApi) {
                    btnCopyApi.addEventListener('click', function () {
                        const apiKey = this.getAttribute('data-api');
                        if (!apiKey) {
                            alert('Không tìm thấy dữ liệu cần sao chép');
                            return;
                        }
                        ...
                        navigator.clipboard.writeText(apiKey).then(function () {
                            ...
                            if (typeof notification !== 'undefined') {
                                notification('success', 'Đã sao chép vào clipboard!', 'Thành công');
                            } else {
