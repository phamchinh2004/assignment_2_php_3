                        ...
                        <div class="user-item d-flex align-items-center p-2 rounded-3 mb-1 cursor-pointer"
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="text-muted" style="font-size: 10px;">
                                    ...
                                    {{ $user->latestConversation->updated_at->diffForHumans() }}
                                    @if($user->latestConversation->messages && $user->latestConversation->messages->count() > 0)
                                    - {{ Str::limit($user->latestConversation->messages->first()->content, 30) }}
                                    @endif
                                </div>
                                @else
                                <div class="text-muted" style="font-size: 10px;">
                                    <i class="fas fa-circle text-secondary me-1"></i>Chưa có tin nhắn
                                </div>
                                @endif
                            </div>
                            @if($user->latestConversation && $user->latestConversation->updated_at->gt(now()->subMinutes(30)))
