<?php
return [
    "CapDoThanhVien" => "Cấp độ thành viên",
    "CapDo" => "Cấp độ: ",
    "SoLuongDonHang" => "Số lượng đơn hàng: ",
    "Unit" => "(Unit)",
    "ChiTietNguoiDung" => "Chi tiết người dùng",
    "HoaHongNhieuHon" => "Hoa hồng nhiều hơn",
    "NhiemVuNhieuHon" => "Nhiệm vụ nhiều hơn",
    "HoTroKhachHangVip" => "Hỗ trợ khách hàng VIP",
    "Ban" => "(Bạn)",
    "SoLuotRutTien" => "Số lượt rút tiền: ",
    "Ngay" => "/ngày",
    "SoTienRutToiDa" => "Số tiền rút tối đa: ",
    "SoLuongNhiemVu" => "Số lượng nhiệm vụ: ",
    "TiLeHoaHong" => "Tỷ lệ hoa hồng: ",
    "ThanhVienVinhVien" => "Thành viên vĩnh viễn",
    "QuayLai" => "Quay lại",
    "BanChuaCoGianHang" => "Bạn chưa có gian hàng",
];
